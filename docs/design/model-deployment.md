...
1. **Resource constraint:** The usage of resource for the model deployment is constrainted by group resource quota.
1. **Ingress:** Create the ingress resource to redirect external requiest to internal model service.
...
The `PhSchedule` resource has following children
...
For each deployment, it requires to provide the model image. It is responsible to translate the REST request to internal model prediction call.
...
Currently, primehub model deployment ONLY support language wrapper solution.
...
And the input and output of the prediction endpoint is *tensor* or *ndarray*.

You can also send a unstructured data (e.g. image file), please find more examples in our [model deployment examples](https://github.com/InfuseAI/model-deployment-examples)
...
The pod of the model deployment has the label `primehub.io/group=escape(<group>)`. The primenhub resource validating admission webhook would invalidate the pod creation if the new pod exceeds the group resource. 
