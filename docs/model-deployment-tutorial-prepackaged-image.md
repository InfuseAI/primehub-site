...
4. Fill in the `Deployment name` field with `quickstart-mnist`
   ...
   Fill in the `Model URI` field with `XXX`; This path is included the trained model in PHFS.

   Fill in the `Model Image` field with `XXX`; This image is a pre-packaged base image that can serve TensorFlow2 model.
