---
id: version-3.4-model-deployment-prepackaged-server-tensorflow2-cht
title: TensorFlow Model Server
description: TensorFlow Model Server
sidebar_label: TensorFlow server
original_id: model-deployment-prepackaged-server-tensorflow2-cht
---
...
支援 TensorFlow2 輸出的 [SavedModel format](https://www.tensorflow.org/guide/saved_model)。 Model URI 檔案結構如同 `tf.saved_model.save()` 輸出結果。
...
└── model.h5
```
**model.h5**: 檔案可為任意檔名加副檔名`.h5` 的 HDF5 format。
...
範例採用 [tensorflow tutorial](https://www.tensorflow.org/tutorials/quickstart/beginner) 裡的 [Keras MNIST dataset](https://www.tensorflow.org/api_docs/python/tf/keras/datasets/mnist)。
