---
id: version-3.5-model-deployment-tutorial-package-image-r
...
- 建立 `mnist.R` 檔，內容可以參考以下的格式內容
    ...
    - 檔名名稱 `mnist.R` 需與在 `Dockerfile` 下的 MODEL_NAME 一致
    ...
    - 另外可以參考 [keras 範例](https://github.com/InfuseAI/model-deployment-examples/tree/master/r_keras_mnist)

## 建置映像檔

建立映像檔的工作資料夾需包含：`install.R` 、 `Dockerfile` 、 `R scripts` 以及 `model file`。
...
透過 curl 進行測試 (其中 `ndarray` 的內容請根據你的應用給予不同的值)：
